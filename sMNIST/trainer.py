...
class Trainer(object):
    ...
    def train_epoch(self, input_data):
        ...
        data = input_data.get_epoch_iterator()



        for i in range(n_batches):
            data_in = next(data)
            self.train_batch(data_in[0],data_in[1], self.lr)
            if ( ((i%500)==0) & (i>0) ):
                print("iter:",i)
                self.validation_ER = self.test_epoch(self.valid_set)
                self.test_ER = self.test_epoch(self.test_set) 
    ...
    def test_epoch(self, input_data):
        ...
        for i in range(n_batches):
            data_in = next(data)
            error_rate += self.test_batch(data_in[0],data_in[1])
    ...
    def build(self):
        ...
        self.train_batch = theano.function(inputs=[x,y,lr], updates=self.model.parameters_updates(x,y,lr), name = "train_batch", on_unused_input='warn')
        self.test_batch = theano.function(inputs = [x,y],  outputs=self.model.errors(x,y), name = "test_batch", on_unused_input='warn')
