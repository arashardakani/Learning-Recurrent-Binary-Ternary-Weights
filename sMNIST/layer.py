...
class LSTM(object):
    ...
    def fprop(self, x, can_fit, eval):
        ...
        xtilde = T.dot(x, self.Wbx)
        Trng = MRG_RandomStreams()
        h_prime = Trng.normal((xtilde.shape[1], self.n_units), std=0.1)

        dummy_states = dict(h=T.zeros((xtilde.shape[0], xtilde.shape[1], self.n_units)),
                            c=T.zeros((xtilde.shape[0], xtilde.shape[1], self.n_units)))
        ...
        def stepfn(t, x, dummy_h, dummy_c, h, c, Wba):

            atilde, btilde = T.dot(h, self.Wba), x
            ...
            h = T.cast(dummy_h + o * T.tanh(c_normal), theano.config.floatX)
        ...
        sequences = [t, xtilde, dummy_states["h"], dummy_states["c"]]
        non_sequences = [self.Wba]
        outputs_info = [
            T.repeat(self.h0[None, :], xtilde.shape[1], axis=0) + h_prime,
